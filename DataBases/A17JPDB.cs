...
namespace AtelierLaDiDa.DataBases
{
    ...
    class A17JPDB
    {
        ...
        /// 資料庫路徑
        /// </summary>
        private string dBPath = "DataBases/atelierLaDiDaA17JPSqlite.db";
        /// <summary>
        /// 連線路徑
        /// </summary>
        ...
        private List<string> seperateSource(string itemName)
        {
            ...
            commandS.CommandText = $"SELECT * FROM Sophie$ WHERE Name = '{itemName}'";
        ...
        private List<string> seperateDestination(string itemName)
        {
            ...
            commandD.CommandText = $"SELECT * FROM Sophie$ WHERE Name = '{itemName}'";
        ...
        private List<string> searchIngredient(string itemNameOrType)
        {
            ...
            SQLiteCommand command = connection.CreateCommand();
            command.CommandText = $"SELECT * FROM Sophie$ WHERE Name = '{itemNameOrType}' OR Type1 = '{itemNameOrType}' OR Type2 = '{itemNameOrType}' OR Type3 = '{itemNameOrType}' OR Type4 = '{itemNameOrType}';";
            //command.CommandText = $"SELECT * FROM Sophie$ WHERE Name = '{itemNameOrType}'";
            SQLiteDataReader dataReader = command.ExecuteReader();
            while (dataReader.Read())
            {
                if (!dataReader[0].Equals(DBNull.Value))
        ...
        {
            SQLiteCommand command = connection.CreateCommand();
            command.CommandText = $"SELECT * FROM Sophie$ WHERE Name = '{secondItemName}' AND Source1 NOT NULL";
            SQLiteDataReader dataReader = command.ExecuteReader();
            if (dataReader.Read() && !dataReader[0].Equals(DBNull.Value))
            {
                //clearList.Add(dataReader["Name"].ToString());
        ...
        public void testDB()
        {
            string testConnectString = "data source = C:\\DB\\UserData.db";
            SQLiteConnection connection = new SQLiteConnection(testConnectString);
            connection.Open();
            //SQLiteCommand command = connection.CreateCommand();
            ...
            //if(dataReader.Read())
            //{
            //    Name = dataReader["Account"].ToString();
            //}
            while (dataReader.Read())
            {
                if (!dataReader[0].Equals(DBNull.Value))
