...
namespace AtelierLaDiDa.DataBases
{
    class A21DB
    {
        ...
        /// 資料庫路徑
        /// </summary>
        private string dBPath = "DataBases/atelierLaDiDaA21Sqlite.db";
        /// <summary>
        /// 連線路徑
        /// </summary>
        ...
        public List<string> generateObjectList()
        {
            ...
            command.CommandText = "SELECT * FROM Ryza$";
        ...
        private List<string> seperateSource(string itemName)
        {
            ...
            commandS.CommandText = $"SELECT * FROM Ryza$ WHERE Name = '{itemName}'";
        ...
        private List<string> seperateDestination(string itemName)
        {
            ...
            commandD.CommandText = $"SELECT * FROM Ryza$ WHERE Name = '{itemName}'";
        ...
        private List<string> searchIngredient(string itemNameOrType)
        {
            ...
            SQLiteCommand command = connection.CreateCommand();
            command.CommandText = $"SELECT * FROM Ryza$ WHERE Name = '{itemNameOrType}' OR Type1 = '{itemNameOrType}' OR Type2 = '{itemNameOrType}' OR Type3 = '{itemNameOrType}' OR Type4 = '{itemNameOrType}';";
            //command.CommandText = $"SELECT * FROM Ryza$ WHERE Name = '{itemNameOrType}'";
            SQLiteDataReader dataReader = command.ExecuteReader();
            while (dataReader.Read())
            {
                if (!dataReader[0].Equals(DBNull.Value))
        ...
        {
            SQLiteCommand command = connection.CreateCommand();
            command.CommandText = $"SELECT * FROM Ryza$ WHERE Name = '{secondItemName}' AND Source1 NOT NULL";
            SQLiteDataReader dataReader = command.ExecuteReader();
